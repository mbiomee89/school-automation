...
export function EarlyLeaveReview({
  ...
  subtitle = 'مراجعة طلبات الخروج المبكر والموافقة عليها أو رفضها',
  onDateChange,
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-slate-600 dark:text-slate-400">
              {subtitle}
              {canReview && pendingCount > 0 ? ` · ${pendingCount} بانتظار المراجعة` : ''}
            </p>
          </div>
          <button
            type="button"
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <div className="flex flex-wrap items-center gap-1.5">
                      {item.createdByStaff ? (
                        <span className="rounded-full bg-sky-500/15 px-2.5 py-0.5 text-xs font-semibold text-sky-800 dark:text-sky-300">
                          من المدرسة
                          {item.createdByName ? ` · ${item.createdByName}` : ''}
                        </span>
                      ) : (
                        <span className="rounded-full bg-violet-500/15 px-2.5 py-0.5 text-xs font-semibold text-violet-800 dark:text-violet-300">
                          ولي الأمر
                        </span>
                      )}
                      <span
